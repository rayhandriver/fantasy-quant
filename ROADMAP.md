...
**0.12 — THE COMPLETE FREE-DATA RECONCILIATION** ☑ **RUN 2026-08-03 — 9/9 bars PASS, store 27 → 45 tables, 800 tests** *(scoped 2026-08-02; Session DATA-1; **T46 ☑** · **T44 ☑** · **T45 ◐** register half · **T47 opened**; findings in `findings.md` §SESSION DATA-1)*. User request: obtain and perfect *every* free source needed for open-ended micro-detail deep dives, "not just for the categories I mentioned but also for anything I may think of down the line". **★ The finding is architectural, not a missing download: `nfl_data_py` is a frozen wrapper over the `nflverse-data` release assets, and we have been reading the wrapper's surface as the data's surface** (T46 — Phase 0.9's "frozen wrapper hits the dead old path" was the general case, not a one-off). Verified live 2026-08-02: **`pbp_participation` — 2016–2025, play grain — has no wrapper function and is absent from a 16-table store**, and it carries `defenders_in_box` · `offense_personnel`/`defense_personnel` · `offense_formation` · `defense_man_zone_type`/`defense_coverage_type` · `route` · `was_pressure` · `number_of_pass_rushers` · and the **gsis IDs of all 22 men on the field for every play** — i.e. box stacking, coverage scheme, 1–3 TE sets and per-player on-field participation, all of it, free, ten seasons. Also found: **`ngs` is ingested (26,723 rows, 2016–2025) and read by exactly one consumer, with NO `features/` module reading it** (T45), and **there is no `schedules` table** — K2's bye readout takes byes out of `ecr_snapshots` (T44). Substeps: **0.12.1** the release-asset loader (bit-identity control + a negative control) · **0.12.2** the generated inventory/gap register `reference/DATA-SOURCES.md` incl. a **consumers** column and each source's **upstream floor** · **0.12.3** participation at play grain (schema drift 20→26 cols; fill rates stated against the *right* denominator) · **0.12.4** `participation_player_week` (the explode is ~100M rows — **materialize the weekly aggregate, keep play grain as a view**) · **0.12.5** FTN 2022–25, labelled `backtestable: false` · **0.12.6** schedules/QBR/contracts/officials/trades/rosters/win-totals · **0.12.7** reconcile `depth_charts`+`depth_charts_ts`, DOUBLE→INT season cols, one team-code canon · **0.12.8** coverage/fill-rate/floor gates + a declared **PIT class** per table + backup. **★ Upstream floors are permanent and go in the register so they stop being re-investigated: participation 2016 · NGS 2016 · PFR 2018 · FTN 2022 — and with `DEV_SEASONS` = 2014–22, FTN contributes exactly ONE development season.** Spec: `docs/BUILD_PLAN.md` §"Session DATA-1"; 9 pre-registered bars, 3 decisions to ask first. **Ingest all seasons, analyse DEV only — loading 2023/24 does not spend the lockbox, building a feature on it does.**
**0.13 — THE DEFENSIVE & TEAM-CONSTRUCTION RECONCILIATION** ☑ **RUN 2026-08-03 — 0.13.0–0.13.9 COMPLETE, all 10 bars PASS, store 45 → 63 registered tables, 954 tests** *(**T48 ☑** · **T49 ☑** · **T50 ☑** · **T52/T53 opened**; findings in `findings.md` §SESSION DATA-2)*. Originally scoped 2026-08-03. User request, on reading the 0.12 scheme inventory: fill **ALL** the gaps, *"especially defensively … as much data as possible about each and every defensive scheme, defensive playcaller, team construction, safety coverage … deep dive into every single team's entire data construction — what offensive/defensive schemes they play the most, which sets they use, target shares, backfield splits, blitz percentages, literally everything."* **★ DATA-1 banked the plays and left them UNATTRIBUTABLE — this is the attribution half.** Three structural gaps, each verified in the store 2026-08-03: **(1)** there is **no defensive play-caller regime table** — `coaches.csv` is offense-only (204 rows, 48 play-callers, zero defensive attribution), so `situation/fingerprint.py`'s z-scoring / EB-shrinkage-by-regime-length / PARTIAL-pinning apparatus **has no defensive counterpart to run on** (**T48**; ⚠ no free source, same artefact class as `coaches.csv`); **(2)** the **defensive half of participation was deliberately never materialized** — `build_participation_player_week` filters `side='offense'` and says why in its own docstring; the `participation_player_play` view already carries the defensive side and nothing reads it; **(3)** **team construction is four zero-reader tables** — `contracts` (51,793) · `weekly_rosters` (533,275) · `depth_charts_all` (955,989) · `draft_picks` (3,077), i.e. T45/T47's pattern for a third and fourth time. **★★ Plus the cross-cutting hazard: from 2023 the vendor stopped emitting NULLs and started emitting SENTINELS, and the fill-rate gate is structurally blind to it** (**T50**, 🔴) — `was_pressure` 2022 = 31,207 null/13,425 F/5,518 T vs 2024 = **14 null**/38,838 F/7,067 T; `number_of_pass_rushers` 72 zeros → **23,754**. Fill reads **0.38 → 1.00** while meaning inverts, and the gate (i) counts *nonnull*, (ii) fails only on **drops** — this is a rise, (iii) is **whole-table, not per-season**. *A naive blitz rate across 2022→2023 reads as a league-wide scheme revolution that is entirely an encoding change.* Also **T49** — floors are registered per **table** and are a property of the **column**: participation's floor is 2016, but man/zone and coverage-shell are **0.000 in 2016–2017**, true floor **2018** ⇒ **five** DEV seasons of man/zone, not seven. Substeps: **0.13.0** the break map `data/breaks.py` + a two-sided per-season fill gate *(build FIRST; every rate downstream is wrong without it)* · **0.13.1** `reference/defense_coaches.csv` + `defense_lineage.csv` *(the hand-curated long pole; **user sign-off gate**)* · **0.13.2** `defense_player_week`/`defense_team_week` *(fronts, blitz rate, man/zone, pressure)* · **0.13.3** coverage shells + the secondary *(three tiers: charted shell ~49 % 2018+ · derived safety-count **proxy** · alignment depth **does not exist free**)* · **0.13.4** the offensive completion *(formation/personnel/motion/PA/RPO/screen/route-tree/alignment; FTN cols stay `backtestable:false`)* · **0.13.5** `team_construction_season` *(cap by position group, draft capital, age, **continuity**)* · **0.13.6** ST + the K environment *(4th-down & 2-pt aggression attributed to the **head coach**, which `pbp` already carries exactly and PIT — **no ST coordinator table**)* · **0.13.7** the unified `team_scheme_season`/`team_scheme_week` panel *(~120–150 cols, z-scored within season)* · **0.13.8** extend `fingerprint.py` to defense *(**descriptive only**, not wired into optimizer/value_board/cost_report)* · **0.13.9** per-column floors, PIT classes, register refresh, backup. Spec: `docs/BUILD_PLAN.md` §"Session DATA-2"; **10 pre-registered bars** (headline **B2: prove the trap is real *first*, then prove the gate closes it**), 3 decisions to ask first. **Derive all seasons, analyse DEV only — deriving 2023/24 defensive facts does not spend the lockbox, building a feature on them does.** **No feature is built here**; M-1 still owes T45+T47. Gates the M-0 … M-6 mining program and the team-by-team deep dives.
...
- **✅ Session DATA-1 — Phase 0.12, the complete free-data reconciliation — RUN 2026-08-03, all 9 bars
  PASS** *(store 27 → 45 tables; 800 tests, ruff clean; **T46 ☑** release loader · **T44 ☑** schedules/byes ·
  **T45 ◐** register half · **T47 opened** — `pfr_*` unread. Findings in `findings.md` §SESSION DATA-1.
  Decisions taken: play grain · one DuckDB file · all small sources.)* *(scoped 2026-08-02,
  docs-only; spec `docs/BUILD_PLAN.md` §"Session DATA-1"; register **T44** · **T45** · **T46**)*. The user
  asked for micro-detail deep dives (box counts for RBs, coverage for WRs, 1–3 TE sets, play-calling
  tempo) and the audit found the binding constraint is **ingest architecture, not modelling**: the frozen
  `nfl_data_py` wrapper is our ceiling, and behind it sits **`pbp_participation` 2016–2025 at play grain**
  — every one of those four questions, plus the 22-man on-field record — never ingested because the
  wrapper has no function for it. Nine pre-registered bars, the headline one being **B4: the four
  questions that motivated the session each return an answer, on real data, end to end** — *a source is
  not ingested until the question that motivated it returns an answer.* **Three decisions to ask first**
  (participation grain — recommend **play**; storage shape — recommend **one DuckDB file**; small-source
  scope — recommend **all**). ~600–900L / 8–12 files / ~15–20 tests. **No feature is built here** — wiring
  NGS into `features/` is M-1 and needs its own bars. Gates the (unscoped) **Sessions M-0 … M-6** mining
  program; independent of VH's two open decisions and of MM-1/MM-2, which stay unblocked.
- **✅ Session DATA-2 — Phase 0.13, the defensive & team-construction reconciliation — RUN 2026-08-03,
  all 10 bars PASS** *(scoped 2026-08-03, docs-only, nothing ran; spec `docs/BUILD_PLAN.md` §"Session DATA-2";
  register **T48** · **T49** · **T50**)*. The user asked to fill **all** the gaps the 0.12 inventory
  surfaced — *"especially defensively"* — before any team-by-team deep dive begins. **DATA-1 banked the
  plays and left them unattributable; this is the attribution half.** The binding constraint is that
  **there is no defensive play-caller regime table** (T48): `coaches.csv` is offense-only, so
  `fingerprint.py`'s whole regime apparatus has no defensive counterpart and every defensive scheme fact
  in the store is person-anonymous. Beside it, the **defensive half of participation was deliberately
  never materialized** (`build_participation_player_week` filters `side='offense'` — right for DATA-1,
  and exactly what blocks DST now), and **team construction is four zero-reader tables** (`contracts`,
  `weekly_rosters`, `depth_charts_all`, `draft_picks`) — T45/T47's pattern for a third and fourth time.
  **★★ The headline finding is an instrument defect**: from 2023 the participation vendor stopped
  emitting NULLs and started emitting **sentinels**, so fill rates climb 0.38 → 1.00 while meaning
  inverts, and `fill_rate_gate` cannot see it on three independent counts — it measures *nonnull*, it
  fails only on **drops**, and it is **whole-table, not per-season** (**T50**, 🔴). *The gate written to
  catch `ngs_air_yards` silently going to zero is blind to the exact opposite failure, and the opposite
  failure is the one actually in the data the mining program exists to mine.* Ten pre-registered bars,
  the headline one being **B2 — prove the trap is real first, then prove the gate closes it**; a fix
  whose effect is unmeasured is a claim, not a fix. **Three decisions to ask first** (defensive regime
  scope — recommend **mirroring `coaches.csv`**; ST coordinator table — recommend **skip**;
  `base_front`/`coverage_identity` — recommend **derived, not curated**). ~700–1,000L / 9–13 files /
  ~20–25 tests **plus a research-and-review block for 0.13.1 that is the real long pole** and needs a
  user sign-off gate exactly as `coaches.csv` did. **No feature is built here.**
  **★ RUN 2026-08-03 — all 10 bars PASS.** Four user decisions taken up front: straight through ·
  0.13.1's historical rows **accepted at recorded confidence** (2026 half already signed off) ·
  0.13.4 built with FTN asserted `backtestable:false` · leave uncommitted. **Shipped:** `data/breaks.py`
  · `data/teams.py` · `situation/{defense_coaches,defense_scheme,coverage,offense_scheme,construction,
  special_teams,scheme_panel,defense_fingerprint}.py` · `reference/defense_coaches.csv` (153 rows) ·
  16 new tables + 2 views, registry **45 → 63**. **954 tests (was 843), ruff clean, B0 PASS**
  (27 baseline tables unchanged, 0 unclassified changes), backup checksum-verified.
  **T48 ☑ T49 ☑ T50 ☑; T52 · T53 opened.** ★★ **The three worst hazards were in the instruments, not
  the data**: the break map is blind to two of the session's three encoding seams (one over its
  cardinality ceiling, one under its conservation test → **T52**); a drop-reason guard passed while
  six LAR regime-seasons silently vanished from a join (*a reason that is always available is not a
  reason*); and four of my own bars measured something other than what they claimed. **Corrections to
  the spec:** slot/wide/inline alignment **does not exist free** (`offense_positions` is the listed
  roster position, not where a man lined up) → registered as a floor beside PFF safety depth; the
  route tree is **19** values, not 14; `contracts.team` is an OTC **nickname/career-path**, never a
  team code, so the roster names the team and the contract names the money. Face validity on the new
  defensive fingerprint: **Bowles +1.14 blitz, Spagnuolo +0.59 man, Quinn +0.77 man.**
  **★ The team-by-team deep dives are now unblocked** — `team_scheme_season`/`_week` (190 registered
  columns, z-scored within season, every column carrying a per-column floor and a PIT class) is what
  they read.
- **The mining program (Sessions M-0 … M-6) is UNBLOCKED but still UNSCOPED**, and **M-1** now
  owes *two* wirings, not one: NGS (**T45**) **and** `pfr_pass`/`pfr_rec`/`pfr_rush` (**T47**) — same
  session, same bars, because both change the exposure matrix the rookie ridge, the QuantReg fits and the
  Phase-6 softness regression read. Also open and independent: **VH's two decisions** (`blend_50`, the
  window). The user's call which comes first.
- **☑ MM-1a — the belief half of the fitted manager model (16.18e), 2026-08-05.** The filled top-150
  elicitation workbook became `reference/manager_profiles/mm1_2026.json` and a **`fitted_manager`
  seat**, which takes one `balanced` seat's chair in `REALISTIC_ROOM`. `FittedManager` loads a
  **profile file** and nothing in the code knows whose it is, so the mechanism is general and the repo
  owner is subject #1 — which preserves the 2026-07-23 "nothing personal to the user" exclusion rather
  than overriding it. **Beliefs only:** `policy_source='corpus'`, asserted. Headlines — his stated
  board is **orthogonal to our own value board** (Spearman +0.008), so it buys *fidelity*, not skill;
  he is genuinely **RB-averse net of board depth** (QB +0.64 / TE +0.59 / WR +0.45 vs RB, t 2.2–2.8);
  the belief **scale is unresolved** (three statistics, three winners on n=40) and ships conservative,
  while the **direction is resolved** (paired bootstrap −0.60 ranks, CI [−1.03, −0.20] vs the
  consensus null; leave-one-draft-out 7.79 vs 8.78). `findings.md` §"SESSION MM-1a".
- **☐ Session MM-1b — the elicitation page + the policy fit (16.18a–c).** A **Streamlit** A-or-B page
  over real board states, actively selecting the pair the posterior is least sure about, with a random
  control arm interleaved; then a conditional logit on his **deviation from the corpus β**, EB-shrunk,
  reporting per-coefficient shrinkage. ⚠ The 45 mock picks are now **partly spent** (one scalar, under
  LOO) — B4's primary arm should become the withheld comparisons. Also owed from MM-1a: **bar B7**, the
  realism before/after for the moved room.
- **☐ 14.P — the personality builder, as a product feature** *(user request 2026-08-05)*. Every user
  builds their own profile in the browser: the belief board as an editable table, the MM-1b pairwise
  page, and a seat selector that drops *your* profile (or a friend's) into any chair of the 16.17
  `SeatMap`. Pure surfacing — the mechanism is already general. ⚠ A personal board must **never** reach
  the cost report's baseline, and the app must never claim a personalized seat drafts *better*.
