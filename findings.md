...
## The competitive UI/UX deep dive (2026-08-01) — Sessions UI-1 … UI-4 scoped, docs-only

**Nothing was built.** No `src/` change, no `app/` change, no test-count change (726, ruff clean). The
outputs are `docs/UI-PLAN.md` (the survey + the recommendation register + fourteen non-recommendations),
`docs/BUILD_PLAN.md` §"Sessions UI-1 … UI-4" (the build spec with per-session pre-registered bars), two
new register entries (**T37**, **T38**), and the pointers in `ROADMAP.md` / `CLAUDE.md`.

**Method.** Eleven products surveyed — FantasyPros Draft Wizard, Draft Sharks War Room, PFF's 2026 mock
simulator, 4for4 Draft Hero, RotoWire, Sleeper, ESPN, Yahoo, Underdog, Ultimate Draft Kit, Footballguys —
plus Boris Chen's tier plots and KeepTradeCut as visualisation references. Then `app/` audited page by
page against them, with an element census as the instrument.

### ★ The premise was right and its obvious reading was wrong

The session opened on *"their UIs are much more favorable and usable than ours."* True. But the survey
does not support a **feature** deficit — it supports the opposite:

> **We are not missing analysis. We have more decision-relevant content than any product surveyed. We are
> missing an information architecture.**

The census, over `app/`'s 1,903 lines: 22 `st.dataframe` · 37 `st.caption` · 31 `warning`/`info`/`error`/
`success` · 28 markdown prose blocks · 9 `st.metric` · **0 uses of colour to encode anything** · 0 status
icons · 0 tier breaks drawn. **68 prose blocks to 9 visual elements**, a ratio every competitor inverts.

**The number that settles the direction of the work:** our `ADVANCED` view is **15 columns — two more
than Draft Sharks' full rankings table**, and Draft Sharks' density is the single most-criticised property
in its own category reviews (*"the extensive amount of features and data might be overwhelming"*),
with Footballguys drawing the same complaint. **The two most analytically rich tools in this market are
penalised for being analytically rich.** So the work is ranking, encoding and hiding — **not adding.**
This is the finding that decided all four session scopes.

### ★ Our differentiator is epistemic, and it currently renders as apology

Seven things we ship have **no competitor equivalent**: a validated `P(available)` with an un-drifted
baseline (FantasyPros' Pick Predictor has neither a baseline nor a validation claim) · `COIN` · `censored
floor` · the auditable T27 chain `PROJ → MEAN → AVAIL → BV` · lockbox provenance · the **printed** 14.I
grade weights (every competitor hides its formula) · the personalization cost report. Each one reaches the
user today as a paragraph explaining a limitation.

The design consequence, and the sentence worth keeping: **a censored quantity should look different, not
be described as different.** `censored floor` becomes a hatched bar and a `⌀` glyph with the sentence in
the tooltip. Generalised: honesty gets a *visual language* — badge, chip, hatch, paired probability —
rather than more prose. **The compression is therefore bar-paired**: prose 68 → ≤ 25 **and** all seven
honesty surfaces still rendering, asserted by driving the app. *A compression that makes one of them
disappear has failed, not succeeded.*

### ★ The one genuinely differentiated feature the survey produced

**Cut tiers by band overlap rather than by value gaps.** A tier ends where adjacent 10–90 bands stop
overlapping — which makes a tier a claim of **statistical indistinguishability**, and that is precisely
what `session.coin_flags` already computes for 14.G and then buries behind a checkbox. It promotes our
most-buried honesty column to the board's primary navigation aid. It is Boris Chen's thesis — the
most-cited visualisation in fantasy football, and it is *one chart* — executed on **our own
distributions** instead of on expert ranks. **Nobody in the category can copy it without first building
a distribution stack.** Everything else in the plan is catch-up; this is not.

### Two defects the audit found

1. **T37 — the best asset in the app is behind a collapsed expander, under a docstring that says it must
   not be.** `draft_room._reach_risk` opens with *"it renders inline rather than behind a button: a
   readout you have to ask for is a readout nobody asks for"* and then calls
   `st.expander(..., expanded=False)`. **The prose and the code disagreed and the prose was right** —
   the same shape as K2's `_handcuff_gaps` docstring, and a reminder that a comment asserting a property
   is not the property.
2. **T38 — a raw `st.json` dump shipped in a user-facing page.** `post_draft` renders
   `c1.json(frames["elite"])` under a heading, on the page whose whole job is to be a report card. It
   survived because K2 **moved** the readouts from the room page rather than rewriting them — *moved, not
   copied* is the right rule and it carries the format along with the content.

### What the four sessions are, in one line each

- **UI-1** — pure formatting, nothing enters `session.py`: a theme, position colour from one constant on
  five surfaces, the prose compression + the hatched-bar visual language, the seat strip, T37, T38.
  Done-bar: **the K2 sheet re-runs unchanged.**
- **UI-2** — first `session.py` work: overlap-based tiers, `Δ` + `BARGAIN`, the 14.F construction flags
  moved to **pick time**, and the 15-column `ADVANCED` split into `VALUE` / `RISK`.
- **UI-3** — the workflow: tags unified with the Cost page (a preference formed while drafting can be
  priced), the inline selected-player strip, and **actually drawing the PLAYER-VIEW §5 bars** — today's
  `st.metric` has no bar and no baseline, the largest spec-vs-shipped gap in the repo.
- **UI-4** — polish, plus a mock history that **replays exactly** (T34's stamped seeds make it near-free,
  and PFF's equivalent cannot replay) and a room scouting report from `drift_frames["seats"]`, labelled
  **realism, never prediction**.

Full spec, per-session bars and per-session ⚠ do-nots: `docs/BUILD_PLAN.md`. Evidence, the product-by-
product survey, the fourteen conventions and the fourteen non-recommendations: `docs/UI-PLAN.md`.

## Session UI-1 (2026-08-01) — "it looks like a product": the theme, the palette, the strip, the compression

**State: 737 tests (was 726), ruff clean, all eight pre-registered bars PASS** —
`steps/session_ui_1.py` → `analysis/session_ui_1.json`. **B0 re-ran the K2 sheet in full**, which
re-runs K1.5's, which re-runs K1's: nothing refits, no frozen contract moved, the spent lockbox was
not re-read. Run it: `uv sync --extra ui && uv run streamlit run app/main.py`.

Four decisions were taken by the user before anything ran: the **palette** (Okabe-Ito, colourblind-safe,
over the market convention), the **theme base** (dark), the **prose target** (≤ 25), and how to resolve
the one spec tension — `P(THERE)` on the slim board versus "do not touch `session.py`".

| bar | result |
|---|---|
| **B0** the K1 rule | the K2 sheet re-runs passing in full, K1.5's and K1's nested inside it — **and every changed leaf is classified**, see below |
| **B1** one palette | six hexes, **one home** (`app/palette.py`), `.streamlit/config.toml` asserted equal, **all five surfaces styled**, every chip ≥ **4.88** contrast (AA is 4.5) |
| **B2** the compression | prose **68 → 23** against a ≤ 25 target, the "before" read from `git show HEAD:` |
| **B3** the honesty surfaces | **all seven still render**, asserted by driving the app, both lockbox branches included |
| **B4** the seat strip | chip order == `SeatMap`, one chip on the clock, on-deck == `team_for_pick`, next pick == the optimizer's; **150 / 150 picks** checked |
| **B5** `P(THERE)` | the column **is** `reach_risk_view`'s number (25 rows identical, 15 uncovered rows NaN), **zero expanders** on the draft page, and the CLI has it too |
| **B6** T38 | **zero** `st.json` in `app/`; the elite-fall dump is a titled table |
| **FLOW** | six pages × k ∈ {0,1,4}, zero exceptions; a pick **made by clicking** finished the draft and landed on 14.N |

### ★★ "All bars pass" is a weaker claim than B0 was written to make, and the gap is where a display session hides

The three nested sheets came back **passing** — and **23 of their 538 leaves had moved.** Passing and
unchanged are different statements, and a display session that only checks the first one has checked the
weaker of the two. So B0 gained a second half: it diffs each committed sheet against `git show HEAD:` and
requires **every** changed leaf to fall under a *named* allowance. An unclassified move fails the bar.

| category | n | what it is |
|---|---|---|
| `display_count` | 5 | counts of rendered elements — UI-1 adds a table to 14.N (T38) and a panel to the rail (T37), so `n_dataframes` is **+1** exactly where it should be |
| `entropy` | 8 | T34's OS-drawn seeds and the openings they produce. **A sheet that reproduced these would mean the randomisation had stopped working** |
| `timing` | 9 | wall-clock `worst_ms`, the per-seat vector, and `worst_seat` — its argmax |
| `stat_dict` | 1 | `n_entries` 20 → 21: the attached `P(THERE)` column |

⚠ The `entropy` and `timing` counts are themselves **run-dependent** — they are drawn, not computed — so
the durable claim is the *categories*, not the tallies. `display_count` and `stat_dict` are the two that
should reproduce exactly, and do.

**No model number moved.** Every `differing: 0`, every *150 of 150 identical*, every Brier, grade, cliff,
probability and board value is untouched. That is now a measured statement rather than an eyeballed one —
which matters because the next session to touch `app/` will read this bar's verdict, not this paragraph.

**★ And the classifier immediately earned its keep by failing.** Its first run flagged one unclassified
leaf: `bars.b3.worst_seat`, `upside_chaser` → `reacher`. That is the **argmax** of
`worst_pick_ms_by_seat` — a timing field wearing a categorical label — and the seven seats sit within a
few milliseconds of each other, so *the argmax of a noisy vector is noisier than the vector itself*. It
is an allowance, and it is one an eyeballed "these all look like timings" would have skipped straight
past, because it does not look like a number at all.

### ★ The palette decision, and the trade it makes

The category convention is QB gold · RB red · WR blue · TE orange · K purple · DST green, and matching a
convention a user already has in their fingers is free familiarity. The user chose **Okabe-Ito** instead,
and the reason is measurable: **the convention puts red, orange and gold in one hue family**, so three of
the six positions collapse into one colour for a deuteranope — roughly one man in twelve. A scanning
channel that does not reach a twelfth of your readers is worth less than the familiarity it buys.

**Two treatments, one constant, both theme-independent.** UI-PLAN §S1 proposed a ~35 % tint with
full-strength text; that is right for dark and wrong for light — full-strength `#0072B2` on a 35 %-`#0072B2`
tint over the dark background is a contrast ratio of about **2.4**, and it inverts when a reader presses the
theme toggle. So the board's `POS` cell gets a **solid** hue with auto-contrast lettering (opaque, so its
contrast does not depend on what is behind it), and a cell that merely *mentions* a position gets an `rgba`
tint with **no colour set**, which the browser composites over whichever theme is live.

**⚠ And the ink was got wrong first, by a threshold.** `ink_for` originally chose black or white by a
luminance cut of 0.42; QB (`#E69F00`) has luminance **0.410**, so it lettered the most-drafted position on
the board in **white at 2.3 : 1**. The break-even luminance is **0.179**, not 0.5 — and the way not to have
to know that is to compute both contrasts and take the larger. *A constant chosen by eye is a constant
nobody re-derives when the palette changes.*

### ★ The compression, and why B3 is the bar that matters

68 → 23. Nothing was deleted: explanations longer than a line moved into `help=`, `st.popover` and
`st.expander`, and state-dependent facts became `st.badge` chips. Three genuine deduplications did real
work — one `no_board_error`, one `no_picks_yet` and one `chain_problem` replaced three, three and two
copies of the same sentence. *Deduplicating a message is the same discipline as deduplicating a
derivation.*

**The failure mode of a compression pass is that it succeeds.** A surface that stops rendering looks
exactly like a surface that got tidier, and both look like a smaller diff — so B2 ships **paired** with B3,
which drives the app at k ∈ {0, 1, 4} and reads what actually came out. Two of its checks failed on their
first run and **both were the bar's fault, not the app's**: the censored-floor chip is only on screen when
a censored row is on screen (they are deep-board rows, and the page defaults to 40), and the stat
dictionary carrying T22 and T31 is the *third* radio option, so loading the page never reached it. Both
were fixed by driving further, not by weakening the claim.

### ★★ The lesson this session paid for three times in one afternoon — a grep cannot tell doing from describing

Three separate instruments in this session counted a **mention** as an **occurrence**:

1. B1's palette census read `9` hexes in `palette.py` — three of them in the module's own docstring
   explaining why a 35 % tint fails.
2. B6's `st.json` census read `1` — the hit was the comment recording what T38 replaced
   (`c1.json(frames["elite"])`).
3. B1's five-surface check reported four, and the missing one was the **log**, which renders only behind a
   radio and only when the log is non-empty — with k = 1 the human seat drafts first, so the fixture's log
   was empty and the surface had nothing to colour.

The first two are now AST parses; the third is a fixture fix. In a repo whose comments deliberately quote
the code they replaced — which is most of this one — **a text scan over source is not a measurement of
behaviour**. The general form: *state the claim about the artifact the claim is about* (a string literal
the code evaluates, a `Call` node, a rendered element), not about the bytes that mention it.

### ★ The one deviation from "nothing enters `session.py`", and why it is not a loophole

UI-1's spec says both *"`P(THERE)` joins the slim board"* and *"do not touch `session.py`"*. The user
resolved it: `P(THERE)` is an **existing number in a new placement**, so `session.attach_reach`,
`session.next_pick_info` and `session.team_for_pick` were added there rather than joining a frame inside a
renderer. The alternative — a merge in `app/` — is the T18 / F.5 / T27 family this repo has already paid
for three times, and it would have given the app a column the CLI could not print. `attach_reach` computes
nothing; `next_pick_info` **removes** a copy (`reach_risk_view` now reads it instead of deriving the window
itself); `team_for_pick` **adds** one, and is allowed to only because B4 differences it against
`DraftState.team_on_clock` at **every one of 150 picks**, which is 16.17's precedent, not an exception to
it. B0 is what makes the claim checkable: nothing existing moved.

**⚠ The first version of that control checked 15 picks and called itself exhaustive** — it advanced the
room between checks, so it landed on the human's seat once a round. 15 of 150 is a sample. The whole reason
a second copy of a formula is tolerable here is that the difference is *complete*.

### ★ Two things deferred out of UI-1, deliberately, and named rather than dropped

- **The positional-strength chart** in A6's report card needs per-position value sums per team — a
  genuinely **new derivation**, which is UI-2's character and not UI-1's. Everything else in A6 shipped.
- **The hatched `censored floor` bar.** UI-1 ships the `⌀` chip with the full sentence in its tooltip; the
  *bar* belongs with UI-3's A3, which is where `st.html` quasi-bars get drawn. ⚠ The `FLAGS` **text** was
  deliberately not edited to carry the glyph: K2's bar B4 matches on that string, and a display layer must
  not edit the thing a bar reads.

### ⚠ Method notes worth keeping

- **`--only` now writes `analysis/session_ui_1.partial.json`, not the sheet.** The house pattern
  overwrites the real artifact with a one-bar run — which happened for real this session: a 30-second
  `--only b6` against the **K2** runner replaced its committed eight-bar sheet with one bar, and it had to
  be restored from git. *A partial measurement should not be able to look like a full one.*
- **`st.badge` renders as markdown** (`:green-badge[…]`) and popover / expander / **tab** bodies render as
  their children, so `AppTest` can see all of them; `st.html` is readable at `element.proto.body`. The
  scrape B3 uses reads every render primitive, because after a compression pass the bar most likely to
  fail for the wrong reason is the one checking that nothing vanished.
- **`app/palette.STYLED` ships in the app**, not in the bar — Streamlit encodes a Styler's CSS into the
  Arrow payload rather than exposing it, so "did this surface get coloured" is not readable from
  `AppTest`; counting the call is. Same reasoning as `app/probe.py`: *a probe that only exists under the
  test measures the test.*
- **Noticed, not acted on: `views.roster_panel` has no callers.** It is K1's roster list, superseded by
  `roster_rail` in K1.5 and left behind. UI-1 coloured it along with everything else, which makes a dead
  renderer look live — flagged here rather than deleted, because deleting pre-existing code is outside a
  formatting session's remit and the user should get the call. **UI-4's polish tail is its natural home.**

---

## Session UI-2 (2026-08-01) — "the board answers the question": Δ · BARGAIN · construction flags · the split — and the tier feature is a NULL

**Goal:** UI-2 as pre-registered — S2 tiers (overlap-based) · A4 `Δ` + `BARGAIN` · A5 live construction
flags · B1 split `ADVANCED` into `VALUE`/`RISK` — plus A6's positional-strength chart, deferred out of
UI-1 by name. First session to touch `draft/session.py` since the app began.

**Decisions taken by the user before the run, and not re-litigated:** overlap cut over gap · tiers within
position · `advanced` retained in `session.py` and off the app's control · glyphs in a new `RISKS` column
with `FLAGS` left byte-identical · `BARGAIN` as a card **field**, not a ninth bar · the A6 chart built
here · rule 7 waived, tree left uncommitted.

### ★★ The headline: the one differentiated feature in the whole UI plan does not work, and finding out why corrected a number we have been quoting all along

UI-PLAN §S2 and BUILD_PLAN §UI-2 both specify the same tier rule — *a tier ends where adjacent 10–90
bands stop overlapping* — and both call it **"the single item in this plan that no competitor could copy
without building our distribution stack first."** It was to be the board's primary navigation aid and the
promotion of `COIN` from a buried checkbox to the thing you draft by.

Built and measured on the live 2026 board, **it cuts nothing at any scope**:

| scope | tiers |
|---|---|
| within RB | **1**, over 62 players |
| within WR | **1**, over 83 |
| within QB / TE | **1** each |
| whole board order | **2**, over 244 available |

**Cause 1 — scale, and it is arithmetic rather than football.** The median RB 80 % band is **228 points**
wide. The median gap between adjacent RBs is **16.3 points**. That is **14×**. A pairwise-overlap rule
cannot cut a sequence whose neighbours sit at a fourteenth of their own interval width; every adjacent
within-position pair overlaps *by construction*, so the chain never breaks.

**Cause 2 — the premise is a category error, and it is the more useful half.** UI-PLAN says the design is
*"precisely Boris Chen's thesis executed on our own distributions rather than on expert ranks."* It is
not. Chen clusters **expert rank dispersion** — how much rankers *disagree about where a player belongs*.
We hold a **predictive interval for a season total**. Disagreement about placement is narrow; uncertainty
about outcome is enormous. Two different quantities wearing one name, and the whole feature rests on
their being the same one. **This is T24's lesson** — *a relationship measured on one object is not a
specification for the mechanism that produced it* — arriving on a **visualisation** instead of on a draft
room, and it presented exactly as T24 did: as a plausible design idea with a citation attached.

### ★ The finding that outlives the feature: `146 of 199` conflates two different answers

`session.coin_flags` returns `False` **both** for *these two are distinguishable* and for *we cannot
tell*, and **its own docstring says those are different answers**. Every published statement of the
column then reports the single number — K2's bar B4, UI-PLAN §3.3, the `COIN` dictionary entry, and this
session's own pre-registered B1 anchor. Decomposed on the top 200:
...
| pairs with a band **missing** | **52** |

Read as *"73 % overlap, so 27 % are resolvable"* — which is how it reads — it is wrong by a factor of
fifty. **Of the adjacent pairs we can evaluate at all, 99.3 % overlap.** That is a *stronger* version of
the honesty claim `COIN` exists to make and a fatal one for anything trying to navigate by it. Both
`coin_flags`' docstring and the `COIN` entry now carry the decomposition, and it is ticketed as **T39**.

**⚠ And my own first measurement of this made the same mistake in the other direction.** The probe that
opened the investigation applied `coin_flags` to the raw sequence and read 53 "breaks", which looked like
54 usable tiers — the number that briefly made board-order scope look like the answer. It was 52 missing
bands and one real break. *An instrument that returns one value for two states will mislead whoever
reads it, including the person who wrote it.*

### ⚠ The gap cut is not a fallback either, and the reason is worth writing down

BUILD_PLAN's option (a) is *"a tier ends where `base_value` falls by more than the pool's **local median
gap**."* Built and measured: **a threshold at the median is exceeded by half of all pairs by
definition**, so it returns ~n/2 tiers (31 over 62 RBs) whatever the data says. It is a coin flip wearing
a tier's name. Worse, cutting on **board** order compounds it — the board is ADP-sorted and `base_value`
is not monotone in it, so one tier came out spanning 0.0 → 63.5. The drop distribution is heavy-tailed
rather than bimodal (max/median **8–16×** per position; only **1–2** drops per position past mean + 2 sd),
so *how many tiers exist* is entirely a function of where the threshold is put. **There is no
threshold-free answer available from arithmetic on frozen contracts**, which is the character UI-2 was
scoped to — a real 1-D clustering with model selection is modelling, with its own validation, and it
belongs to a session that can pay for it.

**What shipped: nothing.** No `TIER` column on any view, `SLIM` keeps `PROJ`, and `session.tier_series`
ships **runnable and unwired** carrying both cuts — the 16.9 / 16.16 precedent (*built, wired, tested,
default OFF*), so the null stays checkable instead of becoming a sentence in a write-up.

### What did ship, and what each one cost

| step | shipped |
|---|---|
| **A4 `Δ`** | `adp − DraftState.overall_pick`, the engine's own counter subtracted. On `SLIM` and `VALUE`, signed always. |
| **A4 `BARGAIN`** | `adp rank − overall_rank`, both over the **whole** board so it is static. On `VALUE` and, as a chip, on the player card. |
| **A5 `RISKS`** | `⚑⛓🛡⌀◔`. The three roster-shape glyphs are read off **`roster_construction_risk` evaluated on (your roster + him)** — the candidate's own bye row, his own NFL-team row, and the handcuff-gap count; `⌀`/`◔` are a strict glyph encoding of `FLAGS`. **554 ms for 40 rows** at a mid-draft roster — one lineup solve per row, which is why the scope is the rendered rows and why the cost is on the sheet rather than assumed. |
| **B1 split** | `ADVANCED`(15) → `VALUE`(12) + `RISK`(9), via `st.segmented_control`; `RANGES` and `SLIM` unchanged in character. `advanced` stays in `session.py`, off the control. |
| **A6 chart** | `session.positional_strength` — per-position **starting** value per team vs the room median, drawn as a diverging bar on 14.N. |

### ★ Four method notes, each of which cost a real failure

**1. A renamed display string is a breaking change even when nothing imports it.** Making the mode
control's option *values* the lowercase mode ids was the obvious build. `board_mode` is a **widget key**,
so it is session state — and UI-1's bar B3 reaches the RANGES view by pre-setting it to `"RANGES"`. Under
lowercase ids that assignment matched no option, the control silently fell back to its default, and B3
reported that **COIN and the censored floor had stopped rendering**: a *"the compression deleted an
honesty surface"* alarm caused entirely by a renamed enum. The fix is that the option values stay the
uppercase labels and the lowering happens at the boundary. *A string anything else can write is an
interface.*

**2. `BARGAIN`'s sign contradicts its own spec, so the spec's words won over its expression.**
BUILD_PLAN and UI-PLAN both write `BARGAIN = overall_rank − adp_rank`. Under that expression a player the
market takes 50th and our board ranks 20th scores **−30** — the column is most negative for the best
bargains, which contradicts PLAYER-VIEW §5's governing rule (*green = good for the drafter, always*) and
its own worked example (*"+1.5 rounds of value"*). Neither document states a polarity in words. Shipped
as `adp_rank − overall_rank` so positive means value, with the discrepancy recorded in the bar sheet, the
docstring and here rather than silently resolved.

**3. A glyph that reads a *maximum* answers a narrower question than the one it is documented to
answer — and only the control noticed.** The first build of `⚑` fired when `max_bye_starters` went
**up** between (roster) and (roster + him). That is a clean delta of the post-draft function's own
scalar, and it is wrong: it fires only when the candidate joins the **already-largest** bye cluster, so
a player who would put a second starter on a clean week showed nothing. Measured on the live board it
fired **zero times in 40 rows**, and the thing that caught it was the bar's *control* — the clause
requiring each glyph to fire at least once — not the equality check, which passed perfectly against an
equally wrong expectation on both sides. Both moved to the documented meaning (*his own row in the
hypothetical readout holds ≥ 2 starters*). ⚠ **A second fixture lesson rode in with it:** at 60 picks
the seat's roster is full and every candidate is a bench add who cannot crack the starting nine, so
`⚑` *cannot* fire correctly at that depth. The bar's draft depth is now part of the bar. That is UI-1's
empty-log lesson for the third time — *a fixture that cannot exhibit the thing being measured is not a
fixture.*

**4. A colour chosen by eye is a colour nobody re-derives when the surface changes.** The first
green/red pair for `Δ`/`BARGAIN` (`#1B7F4B`/`#A32B22`) was picked by eye and failed twice when measured:
the red sat at contrast **2.63** on the app's own dark background — under the 3.0 a *mark* needs — and
**both** poles were under 4.5 as **text**, on both themes at once, because no single mid-tone clears AA
against near-black and white simultaneously. That is UI-PLAN §S1's "35 % tint with full-strength text"
trap, one session later. Fixed the way UI-1 fixed it: the cells get a **tint with no text colour set**,
and the two hues were re-picked by search against all three surfaces — worst-case contrast **3.39**,
normal-vision ΔE **27.0**, deuteranopia **21.1** / protanopia **8.1** / tritanopia **24.9**. They survive
because they differ in **lightness** as much as in hue, which is what makes a red/green pair legible at
all. ⚠ The A6 chart deliberately does **not** inherit `chartCategoricalColors`, which UI-1 set *so that
future charts would*: that was right for a categorical chart, and this one is **diverging** — position
identity is already carried by the axis label, which is exactly what frees colour to carry the sign.

### ⚠ Do not re-derive / do not "fix"

- **`FLAGS` is byte-identical and must stay so** — K2's bar B4 matches on its text. `RISKS` is a second
  column: the *scan* channel beside the *read* channel. `⌀`/`◔` appear in both deliberately.
- **`roster_construction_risk(..., roster=)` adds no arithmetic** and its default path is unchanged,
  which is what keeps K2's 14.F bar and this session's B0 unmoved. The glyphs are deltas of its scalars,
  never a cheaper look-alike rule.
- **`RISKS` is scoped to the rendered rows and that is not the cliff/tier error.** A tier is a fact about
  the pool; a construction flag is a fact about the *pair* (your roster, this player).
- **`advanced` stays in `session.py`.** K1.5's and K2's committed sheets difference against
  `project_view(advanced=True)` returning exactly `BOARD_VIEW_COLS`.
- **`session.tier_series` is a null, not an unfinished feature.** Re-adopting it needs T39's modelling
  session, not a threshold.

---

## Stage-0 pull + reconciliation (2026-08-01, session 4) — the chore broke the controls, correctly

**Goal:** run the Stage-0 FFC snapshot and reconcile everything downstream of the live board. Chore-only
in intent; no model change, no refit, no lockbox read, and the frozen value stack untouched.

**What the pull banked.** `ffc-20260801`, **1,288 new rows** across 6 configs, gsis match **98.0 %**,
`adp_asof` PIT verified, verdict PASS — then `steps/backup_db.py` to `/mnt/c/…/fantasy-quant-backup`
with checksums. The board was only 2 days old (next due after 08-05), so this was an early pull, not an
overdue one.

**What actually changed on the board:** 247 rows vs 246. **3 arrivals** (Darnell Mooney, Kimani Vidal,
Andy Borregales), **2 departures** (NY Jets Defense, Zachariah Branch), mean **|ADP move| 2.21 picks**,
and **91 of 244 matched players moved more than 2 picks** (largest: Mike Washington Jr. +18.1). Small in
aggregate, and enough to change every draft the room plays.

**The reconciliation, in the order it ran:**

| | result |
|---|---|
| data-health gates | **all PASS**, incl. `adp: live-season snapshot fresh` at age 0 days |
| T32 board-vintage gate (2026) | **PASS — 244 resolved == 244 served**, 0 invisible players; new cache `…_ffc-20260801.parquet` written, cold rebuild 12.9 s |
| 16.5 situation events | gate fired: committed **138** rows vs a derivation of **154** → regenerated, annotations merged forward |
| test suite | **749 passed**, unchanged |
| K1 / K1.5 / K2 bar sheets | **all still pass their own bars** on the new board |
| UI-1 / UI-2 sheets | **B0 fails** (T41); **UI-2's B3 fails on its control only** (T40) |

★ **The finding: the chore is scheduled, and so is the alarm it raises.** `CLAUDE.md` §2 mandates a pull
whenever the board is >6 days stale. Nothing in a bar sheet records **which board it was measured on**,
and `_ALLOWED_MOVES` has no category for *the input moved* — so B0 went from `all_pass: True` with zero
unclassified leaves to **122 unclassified in K1's sheet and 23 in K2's**, while every nested sheet kept
passing its own bars. The instrument is behaving correctly (a board refresh genuinely is not a display
change) and is unable to say the one thing the operator needs. **This is T32 one level up** — there the
*cache* key omitted the board vintage, here the *control* does — and it has T32's fix: stamp
`board_vintage` into the sheet and classify the moves, never widen the allowance list. **T41.**

★★ **The second finding, and it is K2's lesson from the opposite side.** UI-2's B3 carries a control —
`every_glyph_fired_at_least_once` — precisely so the row-by-row glyph identity cannot pass vacuously; the
UI-1 pointer records that the first build had `⚑` firing **zero times in 40 rows**. On the 07-30 board
`handcuff` fired **exactly once**. On the 08-01 board it fired **zero** times, and B3 flipped to FAIL
with `mismatched: []`, `FLAGS` byte-identical and unknown byes still unknown — *every correctness claim
intact*. K2 wrote that **a test that cannot fail is the same defect as a bar that cannot fail**; this is
its mirror, a control that can fail and does, on **n = 1**, producing a *false failure*. Both come from
letting a sampled fixture decide whether the interesting case is present. **T40.**

★ **Method note — the attribution was measured, not assumed.** The pre-pull sheets were read *before*
concluding anything, and they showed `all_pass: True` with **zero** unclassified leaves on both files.
Without that step the honest report would have been "the board refresh probably did this", which is the
same sentence a real regression writes. This repo has filed three wrong causes on tickets (T13, T24,
T31); the cheap defence is to read the control's own before-state first. Evidence preserved at
`analysis/session_ui_{1,2}.pre_ffc20260801.json`, following the `phase11_opponent_model.pre_t15.json`
convention.

**What was deliberately NOT done, and why:**
- **No bar was tuned into a pass.** Both failures are reported as tickets. Widening `_ALLOWED_MOVES`
  would delete B0's purpose; loosening B3's control would tune a bar to a board.
- **Consensus projections were NOT re-pulled** — they are dated **2026-07-05**, 27 days behind the ADP
  board. This is *not* a mismatch created by the pull: the 2026-07-04 reframe makes **value = consensus
  projections** and **availability = ADP** deliberately separate signals, so a fresher board with older
  projections is coherent. Coverage was checked and is fine: **4 of 201** skill rows on the new board
  lack a projection (Gainwell, Aiyuk, Vidal, Mooney), and 3 of those are the unmatched arrivals. ⚠ The
  apparent misses "Patrick Mahomes" and "Oronde Gadsden" were a **name-join artifact** of the check
  itself (`… II` suffixes) — the system joins on `gsis_id` and never saw them. It is still worth its own
  decision: the FantasyPros board is as perishable as the FFC one and there is **no recurring chore for
  it**, only `steps/phase4_1_consensus.py` run once on 07-05.
- **`reference/hype_board.csv` was NOT regenerated.** It is inert by construction (every row
  `reviewed=false`, and `load_hype_board` refuses unreviewed rows), so a stale nomination list cannot
  reach a simulation. Regenerating is safe but would add churn to an already-large uncommitted diff.

**Numbers that moved and are quoted elsewhere:** `adjacent_overlapping_pairs` **146 → 150** — the `COIN`
figure carried in the UI-2 pointer, `docs/TECH-DEBT.md` T39 and `glossary.md`. Its *decomposition* (only
147 pairs evaluable, 52 missing bands) is the durable claim; the count is a board fact and will move
every week.

---

## The `value_hawk` objection (2026-08-01, session 5) — Sessions VH · MM-1 · MM-2 scoped, docs-only

The user reported that `value_hawk` is "consistently underperforming" and "consistently makes picks that
are characteristically uncalled for" — a seat meant to be "one of the most realistic replicas of a
genuinely intelligent and knowledgeable fantasy player" — and proposed rebuilding it as a personality
**modelled on himself**, accepting that this would take many drafts or a new narration system.
**Docs-only: nothing ran, no `src/` change, 749 tests unchanged.**

### ★ The objection has a cause already on file, and it is a behaviour rather than the reporting artifact it was filed as

`value_hawk` is the **Phase-9 greedy in an opponent seat**, maximizing **portfolio CE**. The value path is
**slot-blind** — nothing in `draft/optimizer.py` references starters — and this is the **only seat that
maximizes** that quantity. T28 measured the fingerprint and left it as a display defect:

> the gap `capital − startable` is negative for every seat EXCEPT `value_hawk` (**+152**), the only seat
> that maximizes the sum, so the display defect is real and **seat-dependent** while the objective is not.

The shipped mock shows what that buys (`analysis/mock_16_14R_picks.csv`, seat 8): a **QB2 in round 8**
(ADP 63.3) and a **TE2 in round 9** (ADP 85.2) in a **10-team 1-QB full-PPR** league, with the seat's first
RB in round 6. Correct as capital accumulation; indefensible as a roster.

**And that single cause explains both halves of the user's report**, which is why the answer to *"picks, or
finish?"* mattered: he said **both, and they feel related**. They are the same defect seen from two sides —
the picks read as uncalled for *because* the capital they buy cannot start.

### ★★ The method finding: T28 answered a correlational question and closed an interventional one

T28's bar B5 asked **which roster-value definition best *correlates* with title probability** over 200
seated-reshuffled drafts — `team_value` +0.8382 > `portfolio_ce` +0.8202 > `starter_value` +0.7971 — and
used that ranking to decide what the seat should **maximize**, closing the ticket as a labelling fix.

Those are different questions, and the gap between them is **T24's own durable lesson one level up**:
*a relationship measured on outcomes is not a specification for the mechanism that produced them.* T24 met
it as `|drift| ≈ 2·adp_stdev` — a law true of *realized* drift and monotonically harmful when re-injected
as a per-seat *perception*. Here it is `starter_value`: a worse **predictor** of title probability across
rosters, which says nothing directly about whether a seat that **optimizes** it builds better ones. The
interventional experiment — run the seat on each objective, measure the rosters it actually builds — has
never been run, and it is cheap. → **T42**.

A second reason the correlational bar could not see it, visible in T28's own write-up and not followed:
**it pools ten seats, nine of which do not maximize the quantity at all.**

### Two smaller items in the same path

- **T33** — `make_value_hawk_pick_fn(n_teams=)` gets the **room** size, so the interactive seat the user
  watches prices its context weights ~10 % differently from every seat that has ever been measured, and
  with k human seats the divisor moves with the room shape. **The seat he is judging is measurably not the
  seat that was measured** — T27's divergence, one argument along, and the third instance of that family.
- **The window sweep was never resolved.** `analysis/phase16_14r_value_hawk.json` records
  `sweep_resolved: false` — +13.1 CE against a pooled se of 10.7 — so the shipped window is a **default,
  not a result**, and 16.14R correctly refused to read an argmax off noise.

### ★★ The scoping finding: "mimic me" is two objects, and conflating them is what made it look expensive

The user's own estimate — many many drafts, or narrate every player — is the price of learning **one**
thing when you are actually trying to learn **two**:

| | what it is | instrument | cost |
|---|---|---|---|
| **beliefs** | where he disagrees with consensus about *players* | disagreement-only annotation | **~50 rows**, not 250 |
| **policy** | how he trades value/risk/need/scarcity *given* a board | designed pairwise comparisons | ~300, 45–60 min |

Mock drafting learns both at once from the same thin data. Separated, each is cheap — and the split is the
reframe's own **value-vs-availability** line one level down.

**Three things make the hour sufficient, and all three are properties of machinery that already exists:**

1. **A pairwise comparison and a real 40-way draft pick are the same conditional-logit likelihood** at
   different choice-set sizes. So elicited comparisons and any mock drafts he ever plays **pool into one
   fit** — the channels are not exclusive, and the ongoing option stays open with no rework.
2. **His easy picks teach us nothing.** Most picks in a draft are obvious, and an obvious pick carries
   almost no information about the *tradeoff* coefficients. The informative observations are the ones where
   he is torn — rare and unplanned in a real draft, and the *only* thing a designed pair asks about. This
   is why ~300 designed comparisons are worth more than 20 mocks, despite a 40-way choice carrying more raw
   information than a binary one: **you can design for the informative region, you cannot sample your way into it.**
3. **For every player where he agrees with consensus, consensus already encodes his view** — so the belief
   surface is his *disagreements*, which is a small set.

And the fit is a **shrunk deviation from the corpus β**, not a fresh fit — 16.4's `k = σ²/τ²` EB machinery,
pointed at a manager instead of a play-caller. At n ≈ 300 over ~15 features a fresh fit would be mostly
noise; a well-estimated prior is already sitting in `analysis/phase11_opponent_model.json`.

### ★ The bar, decided before the build because it cannot be decided after

**A model fit on his stated preferences and scored by him will always look right.** This is the repo's
**scoring trap** with a sharper edge: previously the seat optimized our board and was scored on our board;
here the optimizer and the grader are the same human. The only falsifiable bar is **held-out prediction** —
against withheld comparisons *and* against real mock picks never used in fitting — beating the corpus-fit
`balanced` on top-1 accuracy and log-loss, or reporting the null.

Attached to it, and worth stating plainly because it is easy to lose: **a faithful replica is a more
*realistic* seat, not a stronger one.** The spent lockbox already recorded that personalization is
noise-dominated on realized points. The deliverable is room realism and practice value, not edge.

### The exclusion this reverses, and why the reversal is narrow

2026-07-23 (session 3) explicitly excluded "a personal self-model" on the grounds that the backlog should
serve **all** users. That reasoning is **preserved rather than overridden**: `FittedManager` is a
...
# THE DATA AUDIT — what is free, what we have, and what we have been unable to see *(2026-08-02)*

*Docs-only scoping session. No code, nothing ran against the store beyond read-only inventory queries.
Prompted by the user's micro-detail deep-dive request; produced Session **DATA-1** (Phase 0.12), and
tickets **T44**, **T45**, **T46**. Verified against the `nflverse-data` GitHub release API and by reading
sample parquets live.*

## ★★ The finding: we have been reading a wrapper's surface as the data's surface

`nfl_data_py` is a **wrapper** over `nflverse-data` GitHub release assets. Whatever it does not expose is
not merely un-ingested — it is **invisible**, and an absence nobody can see is never prioritised.

This had already happened once and was filed as an accident of one release. Phase 0.9: *"nflverse
restructured stats releases post-2024; frozen `nfl_data_py` hits the dead old path."* Reading that as a
one-off is what let the general case survive for another eleven months of build.

The measurement: the repo publishes **25 release tags**; the wrapper exposes a subset. Absent from a
sixteen-table store, with no wrapper function at all:

> **`pbp_participation` — 2016–2025, one row per play** — `defenders_in_box` ·
...
> all 22 men on the field for every play.**

The user's four examples were opponent box stacking for RBs, coverage schemes for WRs, usage of 1–3 TE
sets, and play-calling tempo. The first three are **columns in that file**, at play grain, for ten
seasons, free. The fourth is derivable from `pbp` and partly built already (`features/environment.py`).

⚠ **A correction, recorded because it was told to the user before it was checked.** Earlier the same day I
told him participation data had been cut off after 2023 and that man/zone coverage was not available for
free. Both were wrong: the release is current through 2025 and the coverage fields are in it. *The claim
was made from recall about a vendor's policy; the check took one API call.*

## Two more, found by asking "who reads this?"

**`ngs` is ingested and effectively unread (T45).** 26,723 weekly rows, 2016–2025. One consumer:
`data/panel.py:93`, `stat_type='receiving'`. **No `features/` module reads the table** — `build_exposures`
assembles its 48 columns from `weekly`, `pbp`, `snaps`, `combine`, `game_lines`, `player_ids`. So
`percent_attempts_gte_eight_defenders`, `avg_separation`, `avg_cushion`,
`rush_yards_over_expected_per_att`, `avg_yac_above_expectation` and CPOE sit on disk, PIT-clean, ten
seasons deep, feeding nothing.

> **★ Why it survived: nothing records a table's consumers.** This is **T22 one level up.** T22's lesson
> was *a column's consumers are not only the models that weight it* — that audit grepped `signal_weights`
> and never looked at the display layer. Here no audit happened at all, because no artifact would have
> shown a table with one reader sitting beside tables with fifteen. **A table nobody reads and a table
> that does not exist are indistinguishable from the outside**, and that is why 0.12.2's inventory ships
> with a consumers column.

**There is no `schedules` table (T44).** Session K2 needed one for the 14.F bye clustering, found none,
and took byes out of `ecr_snapshots` — a rankings feed. The workaround still ships. It is honest on
screen (unknown byes render unknown, never week 0) but the coverage of a first-class immutable fact about
the season silently tracks a third party's board depth.

## The floors, and why they are the important half of the register

| source | earliest | DEV seasons it contributes (DEV = 2014–2022) |
|---|---|---|
| participation | 2016 | 7 |
| NGS | 2016 | 7 |
| PFR advanced | 2018 | 5 |
| **FTN charting** | **2022** | **1** |

The user's read that "pre-2022 of other things" is missing is **right for FTN and wrong for everything
else** — and the distinction is the whole value of writing floors down: one is a gap a session can close,
the other is a fact about the world, and only the register tells them apart without a re-investigation.

**FTN contributing one DEV season is disqualifying for backtest use**, and this repo has twice met what
n≈1 produces (T24's four-season subsample that failed on the full sweep; T40's control that passed on a
single observation). So FTN ships `backtestable: false` as an **assertion**, not a caveat — *a docstring
is not a guard.*

## The denominator, which will be misread if it is not stated

Participation's `route` / `defense_man_zone_type` / `was_pressure` read **~0.38 of all rows** across
2016–2022. That is not 62 % missing — it is the **pass-play share of the row count**: ≈18k charted routes
against ≈19k pass attempts per season, and `number_of_pass_rushers` (0.419) and `time_to_throw` (0.395)
cluster in the same band, which is the tell. Personnel and box are ~0.76 for 2016–22 and **1.00** for
2023–25.

Two genuine holes, both going in the register: `defense_coverage_type` is ~0.50 even in the complete
years, and **`ngs_air_yards` is 0.00 from 2023** — a field that stopped being populated with no
announcement and no downstream error. That last one is exactly the failure the 0.12.8 fill-rate gate
exists for: *silent vendor degradation is indistinguishable from a quiet column until something asserts
the rate.*

## The recommendation the audit changed

The mining program (Sessions M-0 … M-6, sketched in `PLAN.md` §2026-08-02, **not scoped**) should aim at
**weekly** decisions and at the **second moment**, not at season-level draft value. The evidence is this
repo's own record: seven hunts aimed at the draft returned seven nulls; the two aimed at the week both
paid. The season is the unit of independence and DEV holds nine of them — which is the same arithmetic
that demoted "beat ADP" in the first place. Consensus is sharp on the mean and publishes no variance at
all, and our own tails are documented as miscalibrated (unconditional coverage 77 % vs 80 %, sim level
bias −113 pts/team, `SPREAD_KAPPA` four hand-tuned constants). *The hole with no competitor in it is the
one worth mining.*

---

# ★★ SESSION DATA-1 (Phase 0.12) — THE COMPLETE FREE-DATA RECONCILIATION
*(run 2026-08-03. 800 tests (was 765), ruff clean. Store 27 → 45 tables. Lockbox untouched, no
refit, no model number moved. All 9 pre-registered bars PASS.)*

## The session in one line

**The binding constraint was never the modelling, it was that nothing recorded what we had** — and
every defect this session found is that one defect wearing a different hat.

## What was built

| substep | landed |
|---|---|
| 0.12.1 | `data/sources/nflverse_release.py` — direct reader for the nflverse-data release assets, **beside** `nflverse.py`, not replacing it |
| 0.12.3 | `participation` — **478,989 plays, 2016–2025**, schema union 20→26 cols |
| 0.12.4 | `participation_player_week` (**182,303 rows**) + `participation_player_play` as a **view** |
| 0.12.5 | `ftn_charting` — 185,215 plays 2022–25, ships `backtestable=False` |
| 0.12.6 | 13 small sources incl. `schedules`, `contracts`, `weekly_rosters`, `qbr_*`, `officials`, `trades` |
| 0.12.7 | `depth_charts_all`, DOUBLE→INT seasons, one team canon, crosswalk rates |
| 0.12.8 | `data/registry.py` — PIT class + backtestable + floor per table, as **assertions** |
| 0.12.2 | generated `reference/DATA-SOURCES.md` + `analysis/data_inventory.json` |

## ★★ Finding 1 — the wrapper was the ceiling, and it cost ten seasons of play-level data (T46 ☑)

`nfl_data_py` is a **wrapper** over `nflverse-data` GitHub release assets. Anything it does not
expose was invisible — *not unavailable, invisible*. Phase 0.9 met a symptom of this and filed it
as a one-off; it was the general case. `pbp_participation` has no wrapper function, and so ten
seasons of `defenders_in_box` / `offense_personnel` / `defense_man_zone_type` / `route` /
`was_pressure` **plus the gsis ids of all 22 men on every play** were absent from a store with
sixteen tables. It took one API call to find and one afternoon to land.

## ★★ Finding 2 — `depth_charts` was not missing 2025; it was two tables sharing a name

The scoped defect was *"`depth_charts` stops at 2024 and 2025 lives separately in
`depth_charts_ts`."* **That is not what was on disk.** `depth_charts` holds **955,989 rows, and
554,215 of them (58 %) ARE the ts series**, appended during the Phase-0.9 backfill with a **NULL
`season`**. So the table carried two grains — weekly depth charts keyed by `season`/`week`, and a
timestamped snapshot series keyed by `dt` — distinguished only by *which columns happen to be
null*. Any `group by season` silently dropped 58 % of it and reported the data as ending in 2024,
**which is exactly how the scoping mis-read it.**

> *A table that answers a different question depending on which rows you land on is not a table
> with a missing season. It is two tables sharing a name.* The fix makes the grain a **column**
> (`depth_charts_all`, season-complete 2014–2025) instead of an inference.

## ★★ Finding 3 — a join rate needs its denominator exactly as much as a fill rate does

**B3 failed on its first run** at 0.983–0.986 for 2016–2022 and exactly 1.000 from 2023 — a
structured pattern that reads like a decaying-backwards data problem. It is not one. The vendor
emits an **empty placeholder row** (`n_offense = n_defense = 0`, no team, no personnel, no player
lists) for plays `pbp` does not carry, ~780 a season, and stopped in 2023. *There is nothing in
those rows to join with.* On rows carrying participation content the match rate is **1.00000 in
every season** (four exceptions in 344k rows).

This is the session's own B5 lesson — *`route` at 0.38 is the pass-play share, not 62 % missing* —
arriving one level up and nearly being mistaken for a defect. **Both rates are now reported.**
The empty rows are **kept**, not dropped: they are faithful to the source (B2 reproduces the
parquet row counts *exactly*, all ten seasons, zero rows dropped) and `n_offense > 0` filters them.

## ★★ Finding 4 — the consumers column went blind to the exact T45 it was written to catch

0.12.2's first run reported **zero unread tables**. That is UI-1's durable lesson 2 — *a grep
cannot tell doing from describing* — landing on the one column whose entire purpose is T45.
**The step that ingests a table mentions it more than anyone**, so counting mentions declares
every freshly-ingested table well-read. Split into **producers / readers / tests**, and with the
second correction that a file can be **both** (`small.py` writes `schedules` *and* reads it in
`bye_weeks`; an `elif` chain reported `schedules` unread on the very run that closed T44 with it):

- `ngs` → **1 reader** (`data/panel.py`), reproducing T45's hand-found result **independently**;
- **`pfr_pass` / `pfr_rec` / `pfr_rush` → 0 readers each.** Three more T45s, found by the
  instrument built for the first one. → **T47.**
- `player_distributions` → 0 readers (written by `phase5_5_utility`, never read back).

**20 of 45 tables have no reader.** Most are this session's own additions, awaiting M-1 — which is
correct and is why the register reports the number rather than gating on it.

## ★ Finding 5 — five declared grains were false, and a false declaration is worse than none

0.12.6's first run declared keys for 13 sources; **five were not unique** (`qbr_season` +275,
`contracts` +20,219, `trades` +3,300, `weekly_rosters` +3,726, `sc_lines` +3). Two were my error
(`season_type` is part of QBR's grain; `line` is part of `sc_lines`'). **Three are properties of
the source** and are now declared as such via `key_unique=False`:

- `trades` is **one row per asset moved**, not per trade — `trade_id` is a *group* key;
- `weekly_rosters` is player-week-**STATUS** (ACT/TRC/TRT/TRD per player-week);
- `contracts` has **no unique row key at all** — OTC emits byte-identical duplicates (3,339 of
  them; one 2022 practice-squad WR has eleven).

The bar now checks that the **declaration is true in both directions** — a key declared unique
that is not is a trap, and a key declared non-unique that is unique is a stale note.

## ★ B4 — the headline: the four motivating questions each answer from one query

*A source is not ingested until the question that motivated it returns an answer.* On DEV (2022
wk 8), each returns real numbers that pass football sense:

| the user asked | answer |
|---|---|
| **box stacking for RBs** | Derrick Henry faced an 8+ box on **51.1 %** of snaps (mean 7.28, highest in the week); Eno Benjamin **0 %** |
| **coverage schemes for WRs** | Garrett Wilson faced the most man (**37.5 %**); DeAndre Hopkins the least (13.6 %) |
| **1–3 TE sets** | Kittle on the field for **100 %** of SF's 12-personnel snaps; Noah Fant 17 in 12 and **19 in 13** |
| **play-calling tempo** | neutral-script sec/play: **DEN 22.2** (fastest) → JAX 31.3, one-score, Q1–Q3 only |

The third is the one no public site publishes, because it needs the on-field record: not *"how
often does this team use 12 personnel"* but *"when they do, is **this** tight end out there."*

## ★ Corrections to the scope, on the record

- **The explode is ~9.9M rows, not ~100M.** The spec's estimate multiplied by seasons twice
  (46k plays × 22 × 10 seasons — but 46k was already per-season). The real view is 9.9M rows,
  **1.5× the rest of the store**, not larger than all of it. **The decision is unchanged** — it
  stays a view — but the number that justified it was wrong by 10×.
- **Fill rates confirmed as scoped**, with one addition: `ngs_air_yards` is **0.00 from 2023** —
  a field that silently stopped being populated and that nothing would have caught. That is now
  precisely what the fill-rate gate exists for, and the gate is unit-tested on this exact case.
- **`participation` has 0 readers and that is deliberate** — play grain is banked so a future
  question needs no re-ingest; `participation_player_week` is the read surface.

## What did NOT happen (deliberately)

**No feature was built.** Wiring NGS into `features/` is M-1, with its own bars — it changes a
matrix the rookie ridge, the QuantReg fits and the Phase-6 softness regression all read.
**Ingest all seasons, analyse DEV only**: 2023/2024 participation is on disk because *loading it
does not spend the lockbox — building a feature on it does.* The wall stays at the modelling step.
The consensus-projection re-pull was not folded in. `data/sources/nflverse.py` was not touched.

## Bars

**B0 PASS** (25 of 27 pre-existing tables byte-identical; **2 classified allowances**
— `depth_charts`/`injuries` DOUBLE→INT, value-preservation proven *before* the rewrite; all 18
existing `data_health` gates PASS) · **B1 PASS** (`combine` bit-identical through the new path;
**two** negative controls, wrong-asset *and* single-perturbed-cell, because the first only proves
schema detection) · **B2 PASS** (10 seasons, per-season counts exact, 0 dropped) · **B3 PASS**
(1.00000 on contentful rows) · **B4 PASS** (4/4) · **B5 PASS** · **B6 PASS** (floors raise below,
silent at and above) · **B7 PASS** (PIT classes and `backtestable` raise; undeclared table raises)
· **B8 PASS** (45/45 declared, reproducible, 0 hand-edited rows).

---

## SESSION DATA-2 (Phase 0.13) — COMPLETE, 0.13.0–0.13.9, all 10 bars PASS (2026-08-03)

**0.13.0 — the break map. All bars PASS.** `src/fantasy_quant/data/breaks.py` +
`validate.{store_fill_rates_by_season,fill_rate_gate_by_season,encoding_break_gate}` +
`steps/phase0_13_0_breaks.py` + 22 tests. 822 tests, ruff clean, **B0 PASS** (25/27 bit-identical,
2 changed exactly as DATA-1 declared, 0 unclassified).

**★ The detector keys on CONSERVATION, not on the level.** A re-encoding moves mass out of NULL
into one in-domain value, so `Δnull ≈ −Δvalue`; a real trend moves a distribution without touching
the null share, and a real backfill raises fill without concentrating on one value. Requiring the
deltas to *cancel* is the whole discriminator, and it is why nothing in the module names a column.

**★ T50 was opened on two columns. There are FOUR, and the two nobody had are the ones that
matter more.**
- `participation.defenders_in_box` @2023 — verified: the 2024 rows at 0 are kickoffs/punts/XP/FG/
  kneels, i.e. exactly the non-scrimmage set that was NULL in 2022. **Naive box mean 6.37 → 4.87;
  scrimmage-only 6.37 → 6.10.** The naive read overstates a real, modest decline ~5×. Box stacking
  was the user's *first* motivating question, so this is the column the mining program would have
  tripped on first.
- `ftn_charting.read_thrown` @2023 — same shape, a **second vendor**, so this is not an nflverse
  quirk.

**★ B2 — the trap, stated.** Naive league blitz rate 2022 **0.2699 → 2023 0.1300**: a 52 % collapse
that would read as the largest defensive scheme shift in modern NFL history. Gated: 0.2699 →
0.2549 → 0.2703 → 0.2729, flat. Before the break the two series are **bit-identical**, so the gate
is a no-op exactly where it should be.

**★ NEW — T51 🟠: `pfr_rec.td` and `pfr_rush.td` are 100 % NULL from 2024.** 518/518 populated
through 2023, 0/523 in 2024 and 0/531 in 2025. This is the `ngs_air_yards` failure again, in a
table T47 already flags as ingested-and-unread. **And the old gate can never fire on it**: the
whole-table baseline recorded `td = 0.7448` — six full seasons averaged with two empty ones — so
the degradation is already laundered into the norm. *A baseline recorded after a degradation
launders it into the norm.*

**★ Not every break is a vendor artefact.** `injuries.report_status` @2016 is the **NFL abolishing
the "Probable" designation** after 2015 (2,702 rows → zero). The detector cannot tell a rule change
from an encoding change and should not try: both are discontinuities a pooled rate must respect.
Relevant to Phase 12's injury-designation event study, which pools across that boundary.

**★ T49 falls out of the same probe, and is bigger than the ticket.** A column entirely NULL in
2016–17 and populated from 2018 *is* a column whose floor is 2018 — so `BreakMap.floor` measures
per-column floors instead of maintaining a second hand list. **19 columns disagree with their
table's registered floor**, not two: participation coverage (2016→2018, confirming T49), plus
`pfr_pass` ×5 (2018→2019/2024), `schedules` odds (1999→2006), `weekly_rosters` ×2, `depth_charts_all`
×5, `injuries.season_type`.

**★ Two calibration lessons the first draft got wrong, both about thresholds:**
1. *`absent` must mean literally zero non-null rows*, not `>= 0.9999`. At the near-threshold
   definition, pbp's rare-event columns (`forced_fumble_player_2_*`, `fumble_recovery_2_*`) flap
   between absent and observed on a handful of rows a season and reported ~30 breaks that were an
   artefact of the cutoff. **A floor is a categorical claim; measure it categorically.**
2. *A floor transition needs materiality.* Without `MIN_PRESENCE`, `tackle_for_loss_2_player_id`
   "appears" for going from 0 to 2 non-null rows in 400,000 — technically a transition, useless.
3. Also: **views inherit their parent's breaks**, so probing `participation_player_play` alongside
   `participation` reported every finding twice as though independent. Base tables only.

**0.13.1 — the defensive regime table. 2026 HALF ONLY (32 rows); the historical regimes are NOT
in the file yet and 0.13.8 can therefore fingerprint nobody.** New `situation/defense_coaches.py`
**reuses four of the offensive module's functions verbatim** (`head_coach_scaffold`,
`audit_against_pbp`, `move_graph_check`, `playcaller_regimes`/`new_regimes`) — they touch only
`season`/`team`/`play_caller`/`in_house`/`head_coach`, which both schemas share, so the move-graph
logic that found every error in the offensive table is maintained once.

**★ The cross-reference earned its keep on the first pass, twice:**
1. **TB** — the consolidated coaches list gives Tampa Bay's DC as "Danny Smith", a career **special
   teams** coordinator. Verified: the Buccaneers hired **no DC at all** for 2026; Bowles keeps the
   calls with Edwards (pass game) and Foote (run game) beneath him. A column misalignment, caught
   only because two sources disagreed.
2. **BAL** — Anthony Weaver holds the DC title and **Jesse Minter (HC) calls the plays**. This is
   the offensive table's Monken case exactly: *the title is not the job.*

**★ 7 of 32 head coaches call the defense themselves** (BAL, HOU, NYJ, SEA, TB, TEN, WAS) — more
than double the offensive rate, and the reason `hc_calls_defense` is a column rather than an
assumption. 11 transport events (`in_house=0`). Confidence 10 high / 22 med.

**0.13.1 — COMPLETE (the header above is superseded).** The historical half landed:
`reference/defense_coaches.csv` is **153 rows** — 121 historical (2014–2025) plus the 32
user-signed-off 2026 rows, passed through verbatim — **33 play-callers, 52 regimes**, confidence
**78 high / 74 med / 1 low**. The pbp head-coach audit compares the researcher's expectation
against the scaffold on all 121 auditable rows: **0 mismatches**. 2026 coverage: **25 of 32**
play-callers have prior history (median 4 seasons); **7 are first-time**, of whom 3 have no mentor
either and are labelled rather than given an invented prior. Per the user's decision, the
historical rows are **accepted at recorded confidence** rather than put through a correction loop;
0.13.8 carries `confidence` through and reports regime coverage instead of hiding it.

---

## 0.13.4 — the offensive completion

`situation/offense_scheme.py` + `steps/phase0_13_4_offense.py`. `offense_team_week` (5,522 rows,
42 cols) and `offense_player_week` (102,368 rows) — formation, personnel, tempo, the **19-route**
tree and usage *within* a personnel grouping.

**★ The third two-encodings column, and the first one the break map cannot see.**
`offense_personnel` is `1 RB, 1 TE, 3 WR` through 2022 and a full-22 `1 C, 2 G, 1 QB, 1 RB, 2 T,
1 TE, 3 WR` from 2023, exactly like `defense_personnel`. Its naive fill climbs **0.7586 → 1.0000**
at the same 2023 seam that T50 is about — and **0.13.0's map reports no break on it**, for two
independent reasons:

1. `probeable_columns` skips anything above `MAX_CARDINALITY` (64); this column has **1,468**
   distinct strings from 2023 against 106 before.
2. The conservation test could not have fired anyway. A re-encoding that lands on **one** sentinel
   conserves mass (`Δnull ≈ −Δvalue`); this one **spreads across hundreds** of new special-teams
   personnel strings, so there is no single value whose share explodes.

> **★ A detector that keys on a single value is blind to a re-encoding that arrives as a
> population.** The instrument built for the 2023 seam misses the 2023 seam on the column this
> substep is built from.

The measurement that closes it is B2's shape with a different denominator: conditioned on
**scrimmage plays**, `offense_personnel` fill is **1.0000 in every season** (naive break 0.2414 →
gated break **0.0001**). The whole jump is non-scrimmage plays gaining strings in 2023. So
`play_type in ('pass','run')` — which every builder in this session already shared — is a
**provable** no-op on the rates rather than an assumption.

**★ B4 PASSES, after both halves of it turned out to be measuring the wrong thing.**

- **B4a** first divided the offensive resolve count by the plays claiming 11-*and*-11, a numerator
  counted over a wider set than its denominator. It reported a "resolve rate" of **1.016** — *a
  rate whose numerator is not a subset of its denominator is not a rate*, and a bar that can exceed
  1.0 cannot fail. With each side against its own denominator: **1.000000 exactly**, both sides,
  443,596 and 442,557 plays.
- **B4b** compared each exploded table against **11 × plays** and failed at ratio 1.0054. The
  expected side was wrong, not the table: the vendor lists **twelve** men on 2,739 scrimmage plays
  and **ten** on 2,171. *Eleven-a-side is football's rule, not the file's.* Against the lists the
  vendor actually emitted, offense is **1.0000** and defense **0.999–1.0** (the deduped
  double-listings 0.13.2 found), with 4,923 / 4,345 off-eleven plays counted rather than filtered.

**★ Two corrections to the spec, both in the direction of less.**

- **Slot / wide / inline alignment does not exist free and is NOT derived.** The spec asked for it
  "from `offense_positions`" — but that column carries the **listed roster position**, the exact
  fact 0.13.2 learned when a two-way player read as an offensive one. Roster position says a man is
  a WR, never that he lined up in the slot. This is **0.13.3 tier 3 in the offensive register**:
  PFF-paid, registered as a floor, do not chase. The honest free substitutes are NGS
  `avg_cushion`/`avg_separation` and the personnel context the module does derive.
- **The route tree is 19 values, not the spec's 14.** Enumerated in `ROUTES` rather than counted,
  so a vendor adding a 20th surfaces as an unlisted string instead of joining an "other" bucket.
  The route is attributed to the **targeted receiver only** — the vendor gives one route per play,
  and spreading it over the other four eligibles would invent four routes nobody charted.

95 personnel strings parse to no package. All 95 are **kick units** (K/LS/P among the eleven),
which the parser refuses a package label on purpose — 999 plays, of which only **13 are scrimmage**
(fakes). Reported as plays split by population, not as a bare string count.

---

## 0.13.5 — team construction

`situation/construction.py` + `data/teams.py` + `steps/phase0_13_5_construction.py`.
`team_construction_season` (416 rows, 32 teams, 2014–2026, 59 cols) and `contract_season`.

**★★ `contracts.team` is not a team code, and using it would have produced a per-team cap sheet
that is wrong in a way that looks right.** It is an OTC **nickname** ("Ravens", "49ers") for 50,134
rows and a **career-path string** ("ARI/ATL/NYJ") for the other 1,659 — 90 distinct values out of a
32-team league. The architecture that works instead: **the roster names the team, the contract
names the money.** Team membership comes from `weekly_rosters` (real codes, per season, per week)
and the contract joins to the **player**. **88–97 % of rostered players resolve to a contract in
force**, per season, and that rate ships as a column rather than a footnote.

**★★ The store speaks FOUR franchise vocabularies, and "we normalized the team codes" was true of
two of them.** Each was found the same way — a join came up short and the shortfall had a pattern:

| vocabulary | example | where |
|---|---|---|
| pbp / participation (the 32 modern codes) | `LA`, `LV`, `JAX` | `pbp`, everything derived from it |
| nflverse alternates | `ARZ`, `BLT`, `CLV`, `HST`, `SL` | `weekly_rosters`, `snaps` |
| **PFR codes** | `GNB`, `KAN`, `LVR`, `NOR`, `NWE`, `SDG`, `SFO`, `TAM` | `draft_picks` |
| **OTC nicknames** | `Ravens`, `49ers` | `contracts` |

New `data/teams.py` is the pbp-side canon. `nickname_map` **derives** the 32 nicknames from
`teams_meta` rather than hand-listing them (the 16.5 rule: the test is not "is this hard to look
up" but "does a feed we already have contain it"), and raises if a nickname ever resolves to two
franchises after canonization.

> **★ And there are TWO legitimate canons, disagreeing on one team.** `adp/panel._TEAM_ALIAS` is
> the ADP-side canon and collapses `LA`/`STL` → **`LAR`**, because the fantasy boards write LAR.
> `pbp` writes **`LA`** in every season. Applying the ADP canon to a pbp-keyed table renames the
> Rams and the join silently drops that franchise — 32 teams in, 31 out, no error, because a
> missing team is just an absent row. `assert_canons_disagree_only_on_la` pins it so a future edit
> to either is loud. **`fingerprint.assert_regime_coverage`'s own docstring already records this
> failure** ("Sean McVay's entire nine-season Rams tenure went missing on the first run") — the
> repo had met it before, which is why the fix is a named canon and not another local patch.

**★ `weekly_rosters.position` is TWO vocabularies, seam at 2016** — fine-grained (CB/FS/SS/DE/DT/
NT/ILB/MLB/OLB/C/G/T/FB) through 2015, already-grouped (DB/DL/LB/OL) after. Unmapped, the
fine-grained positions fell into **no** group and vanished, which is how **64 team-seasons** came
to have position shares that did not sum to 1. **The break map does not flag this one either** —
at 27 distinct values it is *under* the cardinality ceiling, but the change is a **vocabulary swap
among observed values**, so the conservation test has nothing to conserve.

> **★★ Two of this session's three encoding seams are invisible to the detector built for encoding
> seams** — one over the cardinality ceiling, one under the conservation test. 0.13.0 is not wrong;
> its stated failure mode is null→sentinel, and these are re-encodings of a different shape. But
> "we have a break detector" is not "we would notice a break." → **T52.**

**B7, all three parts.**

- **B7b — draft capital sums back to the pick list at ratio 1.0000 exactly**, and only after the
  PFR codes were mapped: before, 7 franchises' picks failed to join and the identity read 0.8105.
  Every one of 3,077 picks maps to a curve row; max pick 262.
- **B7a took two corrections.** It first measured the **season-cumulative** roster (~102 bodies by
  churn), which has no reason to be 53 — the limit is a **weekly** fact. Then, weekly, it read ~80,
  because `weekly_rosters` carries practice squad (DEV), reserve (RES), cut (CUT) and inactive
  (INA) alongside ACT. `status='ACT'` per week gives **44–57**, season means 47.0–55.1. **2016 is
  an upstream roster-convention transition** (a team-week reaches 94) and 2015 dips to 36 —
  recorded as a floor, not smoothed.
- **B7c — the cap, honestly.** Accounted APY sums to **1.2324** of the cap on average, not 1.0,
  because APY is not a cap hit and a traded player is on two rosters. So `cap_pct_*` is *accounted
  APY as a share of cap* and **`cap_share_*` — normalized within accounted money — is the column a
  deep dive should read**: it answers "how much of its money does this team put in the receiver
  room", which survives the coverage gap where "what fraction of the cap" does not. Shares are a
  valid distribution on all 416 rows (0 not summing, 0 impossible).

`contracts` has **4,794 duplicated groups / 6,384 excess rows / worst group 9**, deduplicated
before anything aggregates. Continuity (snap-weighted, from `snaps`, all three phases) averages
**0.67** returning.

---

## 0.13.6 — special teams and the kicking environment

`situation/special_teams.py`. `st_team_season` (384 rows, 96 head coaches) and `kicking_env_week`
(6,590 rows). **No ST-coordinator table**, per the user's scoping decision: 4th-down and 2-point
aggression are head-coach decisions and `pbp` carries the head coach exactly and PIT, so a third
curated CSV would spend human verification on a variable already attributable.

**★ The attribution cross-check is the bar with content, and it passes at 0.** This module derives
the head coach from `pbp` independently of `reference/coaches.csv`, which 0.13.1 audited against
the same source. **0 mismatches over every shared team-season.** Two independent derivations
agreeing is worth more than either asserting.

**★ The denominator trap here is behavioural, not a vendor encoding.** Unconditional 4th-down
go-rate is **0.1696**; conditioned on the situation that offered the choice (4th and ≤5, opponent's
30 to own 30, inside two scores) it is **0.2957** — **1.74×**. The naive number is dominated by
4th-and-12 from your own 20, where nobody goes, so it measures how often a team faced a hopeless
down rather than how aggressive its coach is. Same shape as B2, different mechanism.

**★ A negative result worth recording: dome weather is NULL, not zero-filled.** Checked rather
than assumed, because this is exactly where T50 would bite: 1,838 dome team-weeks, **0
zero-filled temps, 0 zero-filled winds, 1,836 nulls**. Zero degrees and zero wind would have read
as the coldest, stillest venues in the league. The 2 populated rows are one 2025 game where the
vendor logged outdoor conditions under a closed roof.

⚠ A reporting defect this caught in itself: the first readout printed a dome mean temp of **46.0
°F**, which was the mean of **two rows**. Every mean in the artifact now ships with the count it
was taken over — *a mean without its n is a number without a claim*, which is a fill rate without
its denominator one level down.

---

## 0.13.7 — the unified scheme panel

`situation/scheme_panel.py`. `team_scheme_week` (5,522 × 190 registered columns) and
`team_scheme_season`, each with a `_z` twin z-scored **within season**, plus `team_scheme_columns`
— the per-column register T49 asked for.

**★ Rates are ratios of sums, never means of weekly ratios.** A season blitz rate is total blitzes
over total dropbacks; the mean of seventeen weekly rates differs whenever the weekly denominators
differ, which they always do. Where a stored rate has a stored denominator the pair is
re-multiplied before dividing; where it does not, the aggregate is snap-weighted and named so. FTN
columns weight on **FTN-charted** snaps, not all snaps — weighting a 2022+ column by a 2016+
denominator dilutes it with never-charted plays.

**★★ The distinction the floor bar forced, and it is the constructive half of T50: does this column
measure football, or does it measure our coverage of football?**

- A column that measures **football** (`man_share`, `share_cover_3`) must be **NULL** below its
  floor. A team that was never charted did not play zero man coverage.
- A column that measures **coverage** (`charted_share`, every `*_denom`) is honestly **0**. "None
  of this team's snaps were charted in 2016" is true, and it is the only column that *explains* the
  nulls beside it.

Conflating them made the bar fire on four columns doing exactly the right thing. Verified in the
data: `man_share` and `share_cover_3` are NULL in 2016–17 and populated from 2018; `man_zone_denom`
is 0. **0 floor violations, 0 dead columns**, 39 columns carrying a declared floor.

**★ A NULL z is never filled with 0.** A z of 0.0 means *exactly league average*; filling an absent
measurement with it would put every 2017 team at the league mean for man coverage — the single most
misleading thing this panel could do. `z_before_floor = 0`, and the z-scores are z-scores
(mean 0.0000, sd 1.0000, n=32 per season). Side symmetry is exact: **0 offense-only, 0
defense-only** over 5,522 team-weeks.

---

## 0.13.8 — the fingerprint, extended to defense (T48 closes)

`situation/defense_fingerprint.py`. **26 defensive play-callers over 116 regime-seasons**, 21
metrics; plus a **widened offensive** vector (18 metrics, 42 play-callers, 162 regime-seasons) —
`fingerprint.METRICS`' original 14 read `pbp` + `weekly` only and are left exactly as they were.

**The machinery is reused, not reimplemented.** `eb_weights`, `fingerprints` and
`assert_fingerprints_sane` gained one optional `metrics` argument defaulting to the offensive
vector, so every pre-existing caller is byte-identical and the defensive side passes its own
vector to the *same* method-of-moments shrinkage.

**★★ The LAR/LA canon bug, met a second time by the person who had just read about it.** The first
run dropped **six LAR defensive regime-seasons (2020–2025)** and nine offensive ones, because both
coaches CSVs are LAR-keyed and the panel is LA-keyed. Fixing the join recovered them: defense went
24 → **26** play-callers and 110 → **116** regime-seasons; offense 41 → **42** and 153 → **162**.

> **★★ And it passed the guard — which is the more useful half.** The drop carried the reason
> *"no panel row for this team-season"*, which satisfied "a drop must have a stated reason" while
> being **true of every possible drop**. It distinguishes a floor from a join failure not at all.
> **A reason that is always available is not a reason.** The guard now checks that a drop is
> *below the panel's floor* — something that can be false. All 5 remaining drops are 2014–2015,
> genuinely below the 2016 panel floor.

**★ Face validity, unprompted and checkable against football:** Todd Bowles **blitz_rate +1.14**
over 7 TB seasons (the league's signature blitz-heavy coordinator); Vance Joseph **+1.05**;
Steve Spagnuolo **+0.46 blitz / +0.59 man** over 9 KC/NYG seasons; Dan Quinn **man_share +0.77**.

**★ The shrinkage is doing exactly what the floors imply.** `man_share` — a 2018-floor column with
the fewest seasons behind it — has the largest `k` (2.25) and keeps only **28 %** weight at one
season; `blitz_rate` (k=0.73, 2016 floor) keeps **58 %**. A one-season coordinator sits near the
league mean *because* his coverage history is short, which is the correct behaviour and not a bug.

**Descriptive only, as a guard.** `assert_not_wired_into_the_optimizer` AST-parses
`draft/optimizer.py`, `valuation/value_board.py` and `valuation/cost_report.py` and fails if any
imports this module. A future session that wants the wiring deletes the assertion deliberately.

---

## 0.13.9 — floors, PIT classes, register, backup

Registry **45 → 63 tables** (16 DATA-2 tables + the 2 exploded views, which were the `n_undeclared`
the inventory bar caught). `reference/DATA-SOURCES.md` regenerated (232 lines).

**T49 lands as a mechanism, not a note:** `registry.COLUMN_FLOORS` + `column_floor` +
`assert_column_floor`, and the error names the column *and* how it differs from its table —
`participation.defense_man_zone_type has no data before 2018; requested 2016 (the TABLE's floor is
2016 — this is a per-COLUMN floor, T49)`. The **declared** floors are checked against the
**measured** ones from the break map, because a declaration nothing checks is a note.

- **B5 PASS — all six deep-dive questions return answers**, each from one query, each checkable:
  Saquon Barkley **439 carries / 59.1 %** of PHI's 2024 backfield; Ja'Marr Chase **114 targets in
  11 personnel / 28 % share**; PHI 2024 defense **82.8 % nickel, 42.8 % Cover-1**; LV **49.4 %** of
  accounted money on defense. *A source is not ingested until the question that motivated it
  returns an answer.*
- **B6 PASS** — per-column floors raise and name the column; declared == measured on all 9.
- **B8 PASS** — every DATA-2 table declares a PIT class, and the guard is **written to fail and
  verified failing**: an unlagged draft-feature use of `team_scheme_season` raises.
- **B0 PASS** — 27 baseline tables unchanged (25 bit-identical, 2 carrying DATA-1's named DOUBLE→INT
  allowances), **0 unclassified changes**, 33 new.

**954 tests (was 843), ruff clean.** Backup run and checksum-verified.

---

## ★ The session in one line

**DATA-1 banked the plays and left them unattributable; DATA-2 attributed them — and found that
the three worst hazards were in the instruments, not the data.** The break map is blind to two of
this session's three encoding seams (one over its cardinality ceiling, one under its conservation
test); a drop-reason guard passed while a franchise silently vanished from a join; and four
separate bars of my own were measuring something other than what they claimed — a rate whose
numerator was not a subset of its denominator, an expected value taken from football's rules
rather than the file's, a 53-man check against a season-cumulative roster, and a mean over two
rows printed as a climate. **Every one was caught by making the bar able to fail.**

---

## SESSION MM-1a — the belief board becomes a seat (2026-08-05)

**Goal:** 16.18e — turn the filled `reference/mm1_belief_board_2026_20260802.xlsx` into a loadable
`ManagerProfile` and put a `fitted_manager` seat in the room. **Beliefs only**; the policy half
(16.18a–c) waits on an elicitation session the user has asked for as a **Streamlit page**.

**Shipped:** `src/fantasy_quant/draft/manager_profile.py` · `steps/mm1_profile.py` ·
`reference/manager_profiles/mm1_2026.json` · `analysis/mm1_profile.json` ·
`tests/test_manager_profile.py` (20). `draft/personalities.py` gains `Personality.manager_profile` /
`requires_profile`, the `fitted_manager` seat, `assert_room_profiles`, and one `balanced` seat in
`REALISTIC_ROOM` steps aside for it (user decision).

### ★★ The finding that decides how much to trust the seat: his beliefs are orthogonal to ours

`corr(his Value Score, our Model vs ADP) = 0.135`, **Spearman +0.008** over 149 rated players. The
personal board is **genuinely new information**, not a restatement of the value board with a human
voice on top. The corollary is the honest one and it is the spec's own: *this makes the seat a
faithful replica, not a better drafter.* A channel uncorrelated with our model cannot be expected to
improve our model's answers, and nothing here claims it does.

### ★ He is RB-averse, and it is not a depth artifact

Regressing his signed value score on position + `log(depth)` (n=150, base = RB):
**QB +0.64** (t 2.80) · **TE +0.59** (t 2.24) · **WR +0.45** (t 2.76) · depth **+0.13/sd** (t 1.86).
The naive read — "he just likes deep players, and RBs are shallow" — is **wrong**: the positional
lean survives the control and depth alone does not clear its own noise.

⚠ **It is deliberately NOT expressed structurally** (user decision). The lean was *estimated from*
the per-player deltas, so putting it back as a positional dummy would price the same opinion twice.
Cost, stated: the seat expresses opinions only about the ~150 players actually rated, so it reverts
toward `balanced` as the live board drifts off the workbook. Regenerating the workbook is the fix.

### ★★ The scale is unresolved and shipped conservative — three statistics, three winners

The workbook records **direction and confidence** and no magnitude (`Personal ADP` is empty for all
150 rows), so picks-per-unit had to be calibrated. Against his **45 real Sleeper mock picks**
(2026-07-11, three weeks *before* the workbook — genuinely held out):

| scale | mean rank | median | top-1 |
|---|---|---|---|
| 0 (consensus null, avoids still on) | 8.78 | 4.5 | 0.200 |
| **3 (shipped)** | 8.18 | 3.5 | **0.275** |
| 6 | 7.60 | **3.0** | 0.250 |
| 10 (argmin) | **7.55** | 4.0 | 0.200 |
| 12 | 7.98 | 4.0 | 0.200 |

`mean_rank` falls to 10, `median_rank` peaks at 5–6, `top1` peaks at 3–4. **Three statistics, three
winners, on n=40 scored picks** — so the sweep is reported `sweep_resolved: false` and the shipped
scale is the *smallest* within one paired-bootstrap se of the argmin. VH.3's rule applied verbatim:
*if it is unresolved at achievable n, say so and keep the conservative default rather than reading
an argmax off noise.* The conservative pick happens to also be the best on top-1.

**What IS resolved: the direction.** Paired bootstrap of the shipped scale against the consensus
null over the same 40 choices — **−0.60 ranks, se 0.21, CI [−1.03, −0.20], clear of zero.** And the
procedure evaluated leave-one-**draft**-out (choose the scale on two drafts, score the third) gives
**OOF mean rank 7.79 vs 8.78 null, top-1 0.229 vs 0.200.** ⚠ The fold is a draft, not a pick — picks
inside one draft share a board, a seat and a running roster, and splitting on picks would leave most
of a draft's information in the training set (16.8's sibling-derived-feature failure, on a CV split).

**The unprompted corroboration, computed before any fit:** his 38 board-matched real picks average
Value Score **+0.92** against a board mean of **+0.25**. He drafts what he says is underpriced —
Egbuka R5 twice, Kraft R7 twice, Jadarian Price in all three drafts, Corum R9, LA Rams DEF R14
twice, QBs at R6/R6/R8. Every one of those is written in his notes.

### ★ Three places the stated rules and the realized picks disagree — reported, not patched (B5)

1. **"Blank note = I would not draft him"** collides with exactly one pick: **Jayden Reed R12**
   (blank note, dropdown *Undervalued*, and a Packer — all three stated rules at once).
2. **The Packers rule** ("I wouldn't draft any Packers") is contradicted by **Tucker Kraft R7 in two
   of three drafts** and by Reed. Resolved by the user as *a hard rule with Kraft as the one named
   exception*, which the profile now encodes and a test pins.
3. **Two picks against a stated fade** — Ashton Jeanty R3 and Justin Herbert R8, both marked
   *Overvalued*. A fade is a tilt, not an avoid, so the seat can still take them; it is faithful.

### ★ "Blank = never draft" had to be scoped to the rows he SAW

Applied to the whole draftable universe it forbids **Cameron Dicker, whom he took in all three
mocks** — a kicker was never on the top-150 sheet, so he never declined one. `unrated: "neutral"` is
the rule: *a player who was never listed was never declined.* 35 avoids (29 `no_interest`,
6 `team_rule`) out of 150 rated rows; 78 non-zero beliefs; mean |delta| 3.96 picks, max 9.0.

### ★ The seat is not inert — and that is asserted, not hoped

This repo has shipped five mechanisms that ran, passed a green suite and did nothing. Against the
live 2026 board with a `balanced` in the same chair on the same seed: **the roster differs from
round 3 on** — R3 Chris Olave over Jeremiyah Love (his single biggest fade, "ridiculously
overvalued"), R5 Egbuka, R6 David Montgomery, R10 Tucker Kraft. The control takes Dallas Goedert,
Rachaad White and **Jordan Love**; the fitted seat declines all three, and the only Packer it drafts
is Kraft. **60 of 150 picks change across the whole draft** — one changed seat cascades, T33's
chaotic-amplification finding again, which is exactly why B7 is owed.

### Method notes worth keeping

- **A synthetic fixture caught a real defect the live board could not.** `name_key` stripped every
  non-letter, so any two names differing only by a digit collided. No real board would have shown
  it; a fixture of `Player 1 … Player 40` did, on its first run. Digits are kept now, and
  `build_profile` raises on a collision rather than letting one belief overwrite another.
- **An avoided player's belief is deleted, not merely unused.** A positive delta on a player the
  seat will never consider reads, to anyone inspecting the file, as an opinion the model holds and
  does not act on. 8 beliefs were dropped this way and the count is recorded.
- **`policy_source` is validated.** A profile carrying policy coefficients while claiming
  `policy_source='corpus'` raises — an unfitted seat must never be reportable as a fitted one.

### ★★ The defect that mattered most was invisible to every bar written to catch it

`REALISTIC_ROOM` moved, so the realism sheet had to be re-measured. Both arms ran. Then a question
that no bar asks — *what season is this profile about?* — turned up a **point-in-time violation**:

> The profile describes **2026**. Every measurement harness in this repo sweeps **2017–2024**. The
> workbook was filled in **August 2026** by someone who watched those seasons happen.

So the batch run was feeding the future into a historical measurement — `CLAUDE.md` §3.1's
non-negotiable, and it arrived through *room composition*, a place nobody thinks of as a data path.

**★ Why it hid: the leak is graded.** A profile keyed on player identity fires on whoever is still
on the board, so the 2026 board rated **6** rows in 2017, 21 in 2020, **55 in 2024** and 78 in 2026.
A seat that is nearly `balanced` in the early seasons and increasingly itself toward the present
does not look broken in any single season's output — *it looks like a seat with a mild opinion.*

**★★ And its measured effect is smaller than any bar in the harness can resolve.** The un-gated arm
against the pre-16.18 room, 40 seeds × 8 seasons, `--shuffle-room`:

| bar | pre-16.18 | un-gated 16.18 |
|---|---|---|
| 1 profile distance · round-1 | 0.0894 · 2.640 | 0.0887 · 2.657 |
| 2 elite p95 · past-10 | 16.0 · 25.24 % | 16.0 · 25.16 % |
| 5 dispersion level error | −11.39 % | −11.24 % |
| landing, top band past pick 4 | 15.31 % | 15.16 % |

**All five T15 gates PASS on both arms.** A session that had only read its own pre-registered bars
would have shipped the violation and reported "no movement". *The size of a defect is not the
argument for fixing it* — and a bar that cannot see a defect is not evidence the defect is absent.

**The fix is structural, and it dissolves the ticket it was opened under.**
`ManagerProfile.covers(season)` + `make_room(..., season=)` degrade a `requires_profile` seat to
`balanced` on a season its profile does not describe. It **degrades rather than raises** on purpose:
a historical measurement legitimately wants the room *minus* the seat, and `balanced` is precisely
the chair 16.18 took — so an uncovered season reproduces the pre-16.18 room **bit-for-bit**. That
makes bar **B7** hold by *construction* instead of by measurement, keeps every committed historical
sheet a valid reference, and leaves the seat where it belongs: on the one board its subject rated.

**Proven, not asserted** — `steps/mm1_b7_room_gate.py` → `analysis/mm1_b7_room_gate.json` runs the
gated default room against an explicitly pre-16.18 room (2017+2018 × 6 seeds, `--shuffle-room`) and
