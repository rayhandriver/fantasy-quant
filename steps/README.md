...
Scripts cover **Phase 0 → 8** (`phase0_2_*` … `phase5_5_*`: ingest → harness → baselines → features →
value → distributions; `phase6_adp_bias`: ADP-softness scorecard; `phase8_covariance`: relationship
correlations + shrinkage, handcuff copula, roster risk, and the covariance-aware greedy checks) plus the
**personalization spine** (`spine_1_config` · `spine_2_optimizer` · `spine_3_cost_report` ·
`spine_4_validate` — the realized-PAR validation of the cost number). The spine's UI is the Streamlit app,
